...
This is a list generated by reading the phoneme table.  It lists every Phoneme group described in the table and has a `Pose Action` field for each group that lets you define the mouth position for each group.  You'll need to provide poses for each entery in the table for Parrot to generate lipsync tracks.
...
Parrot uses an editable table that you can use to map spoken syllables (aka: phonemes) to mouth positions.  It works by collecting the many different spoken syllables into groups.  This lets you set mouth positions per group rather than having to create poses for the dozens of different spoken syllables.

If you do not specify a phoneme table, Parrot will use the default one that it ships with.  However if you want to modify it or write your own, you can specify it in the Phoneme Table field to override the default.

You can use the following visual guide for the mouth positions for the default phoneme table:

![Default phoneme table mouth positions](art/images/mouth_positions.jpg)
